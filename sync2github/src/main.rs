...
fn main() {
    let lpath = "/home/jzhang/ws/doc/src/github.com/jzhang/notes";
    let repo = match Repository::open(lpath) {
        Ok(repo) => repo,
    ...
    let statuses =  match repo.statuses(Some(&mut opts)) {
        Ok(statuses) => print_short(&repo, &statuses),
        Err(e) => panic!("failed to get git statues: {}", e),
    };
}
