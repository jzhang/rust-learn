...
fn read_and_validate(b: &mut io::BufRead) -> Result<PositiveNonzeroInteger, Box<error::Error>> {
    ...
    match answer {
        Ok(n)                        => Ok(n),
        Err(CreationError::Negative) => Result::Err(Box::new(CreationError::Negative)),
        Err(CreationError::Zero)     => Result::Err(Box::new(CreationError::Zero)),
    }
}
